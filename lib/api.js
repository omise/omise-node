...
var authOpt = {'headers': {'Authorization': ''} };
var api = module.exports = {
  post: function(options, callback) {
    if (!('path' in options)) {
      throw new ApiError('path and data are required!')
    }
    authOpt['headers']['Authorization'] = httpAuthorization(options['key']).httpBasic();
    postRequest(mergeObjects(authOpt, options),
                callback);
  },
  get: function(options, callback) {
    authOpt['headers']['Authorization'] = httpAuthorization(options['key']).httpBasic();
    getRequest(mergeObjects(authOpt, options),
               callback);
  },
  destroy: function(options, callback) {
    authOpt['headers']['Authorization'] = httpAuthorization(options['key']).httpBasic();
    deleteRequest(mergeObjects(authOpt, options),
               callback);
  }
};
